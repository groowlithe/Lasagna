...
bool Title::load(u64 _id, FS_MediaType _media, FS_CardType _card)
{
	...
	if (loadTitle)
	{
		if (!directoryExist(getArchiveSDMC(), folderPath))
		{
			...
			if (R_FAILED(res))
			{
				createError(res, "Failed to create container directory.");
			}
		}
		
		if (!directoryExist(getArchiveSDMC(), patchPath))
		{
			Result res = createDirectory(getArchiveSDMC(), patchPath);
			if (R_FAILED(res))
			{
				createError(res, "Failed to create patch directory.");
			}
		}
...
{
	directories.clear();
	Directory list(getArchiveSDMC(), folderPath);
	if (list.getLoaded())
	{
...
{
	std::u16string savecachePath = u8tou16("/3ds/Lasagna/cache");
	...
	std::u16string titlesHashPath = u8tou16("/3ds/Lasagna/titles.sha");
...
static void exportTitleListCache(void)
{
	...
	std::u16string path = u8tou16("/3ds/Lasagna/cache");
...
static void importTitleListCache(void)
{
	FSStream inputsaves(getArchiveSDMC(), u8tou16("/3ds/Lasagna/cache"), FS_OPEN_READ);
